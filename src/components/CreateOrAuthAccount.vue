...
function invalidateInput(inputValue) {
...
<template>
    <div>
      <div class=" flex items-center justify-center w-screen h-screen max-[425px]:items-start">
        <div class=" max-[2561px]:max-w-[1980px] max-[2561px]:max-h-[1020px] max-[2561px]:flex-row max-[1981px]:max-w-[1441px] max-[1981px]:max-h-[610px] max-[1441px]:max-w-[1020px] max-[1441px]:max-h-[600px] max-[1025px]:max-w-[800px] max-[1025px]:max-h-[400px] max-[426px]:flex-wrap max-[426px]:max-h-screen flex  w-full h-full border p-4">
          <div class=" w-full border p-4 flex flex-col justify-center items-center h-full max-[426px]:hidden">
            <div class=" flex flex-col w-11/12 h-11/12 border">
              <h1 class=" max-[2561px]:text-8xl max-[1441px]:text-6xl max-[769px]:text-4xl">
                Looking<br>
                Something New
              </h1>
            </div>
          </div>
          <div class=" w-full border flex flex-col justify-center items-center">
            <div class=" max-[426px]:h-[50%] max-[426px]:border max-[321px]:h-[40%] flex flex-col w-[70%] h-[80%] items-center justify-around max-[769px]:p-8">
              <div v-show="isLogin" class=" h-[180px] w-[180px] max-[1981px]:w-[120px] max-[1981px]:h-[120px] max-[1025px]:w-[80px] max-[1025px]:h-[80px] ">
                ...
                <InputForm :serviceEnd="serviceEnd" :inputField=" isLogin ? {username:null,password:null}:{firstName:null, lastName:null, username:null, password:null, email:null}" @assginData="getUserInput">
                  <template #option>
                    <p v-show="isLogin" class=" underline">
                      forget password?
                    </p>
                  </template>
                  <template v-slot:button>
                    <button class=" w-full border pt-1 pb-0.5 pl-2 pr-2 h-full " @click=register>
                      {{ isLogin ? 'login':'signup' }}
                    </button>
                  </template>
                  <template v-slot:option2>
                    <p @click="changeForm" class=" text-center mt-2">
                      <span class=" mr-2">{{ isLogin ? 'new user?': 'have an account?'}} </span>
                      <span class="underline">{{ isLogin ? 'signup': 'login' }}</span>
                    </p>
                  </template>
                </InputForm>
            </div>
